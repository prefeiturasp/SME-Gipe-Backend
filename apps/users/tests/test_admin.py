...
@pytest.fixture
def api_client(django_user_model, cargo):
    def _create(username="tester", **kwargs):
        pwd = secrets.token_urlsafe(16)
        if "cargo" not in kwargs:
            kwargs["cargo"] = cargo
        user = django_user_model.objects.create_user(username=username, **kwargs)
        user.set_password(pwd)
        user.save()
        client = APIClient()
        client.force_authenticate(user=user)
        return user, client, pwd
    return _create


def create_user(cargo, username="user", **kwargs):
    pwd = secrets.token_urlsafe(16)
    if "cargo" not in kwargs:
        kwargs["cargo"] = cargo
...
class TestCargoAdmin:
    ...
    def test_change_view_accessible(self, admin_client):
        cargo_obj = Cargo.objects.create(codigo=6, nome="Engenheiro")
        url = reverse("admin:users_cargo_change", kwargs={"object_id": cargo_obj.pk})
        response = admin_client.get(url)
        assert response.status_code == HTTPStatus.OK
...
class TestCustomUserChangeForm:
    ...
    def test_valid_cpf(self, cargo):
        user, _ = create_user(cargo, username="valido", name="Valido", cpf="98765432100", email="valido@example.com", rede="DIRETA", is_validado=True)
    ...
    def test_invalid_cpf(self, cargo):
        user, _ = create_user(cargo, username="teste", name="Teste", cpf="12345678900", rede="INDIRETA", is_validado=True)
        form = CustomUserChangeForm(
            data={
                "username": user.username,
                "name": user.name,
...
class TestCustomAdminPasswordChangeForm:
    ...
    def test_removes_usable_password_field(self, cargo):
        user, _ = create_user(cargo, username="admin_test", is_validado=True)
...
class TestEnviarParaCoreSSOAction:
    ...
    def create_users(self, cargo, users_data):
        users = []
        for data in users_data:
            user, _ = create_user(cargo, **data)
            users.append(user)
        return users

    def test_render_confirmation_template(self, admin_client, cargo):
        user = self.create_users(cargo, [{"username": "user1", "name": "User 1", "cpf": "12345678901", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "enviar_para_core_sso", "_selected_action": [user.pk]}
    ...
    def test_action_with_valid_users(self, mock_cria_core_sso, mock_enviar_email, admin_client, cargo):
        user = self.create_users(cargo, [{"username": "user_valid", "name": "User Valid", "cpf": "12345678902", "email": "valid@exemplo.com", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "enviar_para_core_sso", "_selected_action": [user.pk], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        messages_list = [str(m) for m in response.context["messages"]]
        assert response.status_code == 200
        assert any("usuário(s) registrado(s) com sucesso no CoreSSO!" in m for m in messages_list)
        mock_enviar_email.assert_called_once_with(
            ...
            contexto={"nome_usuario": user.name, "aplicacao_url": ANY, "senha": ANY},
        )

    def test_action_with_invalid_users(self, admin_client, cargo):
        user = self.create_users(cargo, [{"username": "user_invalid", "name": "User Invalid", "cpf": "12345678903", "rede": TipoGestaoChoices.DIRETA, "is_validado": False}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "enviar_para_core_sso", "_selected_action": [user.pk], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        assert response.status_code == 200
    ...
    def test_action_with_mixed_users(self, mock_cria_core_sso, mock_enviar_email, admin_client, cargo):
        ...
        users = self.create_users(cargo, [
            {"username": "user_valid", "name": "User Valid", "cpf": "12345678902", "email": "valid@exemplo.com", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True},
            {"username": "user_invalid", "name": "User Invalid", "cpf": "12345678903", "email": "invalid@exemplo.com", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True},
        ])
        url = reverse("admin:users_user_changelist")
        data = {"action": "enviar_para_core_sso", "_selected_action": [u.pk for u in users], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        messages_list = [str(m) for m in response.context["messages"]]
        assert response.status_code == 200
        assert any("usuário(s) registrado(s) com sucesso no CoreSSO!" in m for m in messages_list)
        assert any("user_invalid: Erro no CoreSSO" in m for m in messages_list)
        mock_enviar_email.assert_called_once_with(
            destinatario=users[0].email,
            assunto="Seu acesso ao GIPE foi aprovado!",
            template_html="emails/cadastro_aprovado.html",
            contexto={"nome_usuario": users[0].name, "aplicacao_url": ANY, "senha": ANY},
        )
    ...
    def test_action_gera_erro_carga_usuario_exception(self, mock_cria_core_sso, admin_client, cargo):
        ...
        user = self.create_users(cargo, [{"username": "user_erro", "name": "User Erro", "cpf": "12345678906", "rede": "INDIRETA", "is_validado": True}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "enviar_para_core_sso", "_selected_action": [user.pk], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
...
class TestRemoverDoCoreSSOAction:
    ...
    def test_render_confirmation_template(self, admin_client, cargo):
        user = self.create_users(cargo, [{"username": "user1", "name": "User 1", "cpf": "12345678901", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True, "is_core_sso": True}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "remover_do_core_sso", "_selected_action": [user.pk]}
        response = admin_client.post(url, data)
        template_names = [t.name for t in response.templates if t.name]
        assert response.status_code == 200
    ...
    def test_action_with_valid_users(self, mock_remover, admin_client, cargo):
        user = self.create_users(cargo, [{"username": "user_valid", "name": "User Valid", "cpf": "12345678902", "email": "valid@exemplo.com", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True, "is_core_sso": True}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "remover_do_core_sso", "_selected_action": [user.pk], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        messages_list = [str(m) for m in response.context["messages"]]
        assert response.status_code == 200
        assert any("perfil(is) removido(s) do CoreSSO com sucesso!" in m for m in messages_list)
        mock_remover.assert_called_once_with(login=user.username)

    def test_action_with_invalid_users(self, admin_client, cargo):
        user = self.create_users(cargo, [{"username": "user_invalid", "name": "User Invalid", "cpf": "12345678903", "rede": TipoGestaoChoices.DIRETA, "is_validado": False, "is_core_sso": False}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "remover_do_core_sso", "_selected_action": [user.pk], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        assert response.status_code == 200
    ...
    def test_action_with_mixed_users(self, mock_remover, admin_client, cargo):
        ...
        users = self.create_users(cargo, [
            {"username": "user_valid", "name": "User Valid", "cpf": "12345678902", "email": "valid@exemplo.com", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True, "is_core_sso": True},
            {"username": "user_invalid", "name": "User Invalid", "cpf": "12345678903", "email": "invalid@exemplo.com", "rede": TipoGestaoChoices.INDIRETA, "is_validado": True, "is_core_sso": True},
        ])
        url = reverse("admin:users_user_changelist")
        data = {"action": "remover_do_core_sso", "_selected_action": [u.pk for u in users], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        messages_list = [str(m) for m in response.context["messages"]]
        assert response.status_code == 200
        assert any("perfil(is) removido(s) do CoreSSO com sucesso!" in m for m in messages_list)
        assert any("user_invalid: Erro simulado ao remover" in m for m in messages_list)
    ...
    def test_action_gera_erro_carga_usuario_exception(self, mock_remover, admin_client, cargo):
        ...
        user = self.create_users(cargo, [{"username": "user_erro", "name": "User Erro", "cpf": "12345678906", "rede": "INDIRETA", "is_validado": True, "is_core_sso": True}])[0]
        url = reverse("admin:users_user_changelist")
        data = {"action": "remover_do_core_sso", "_selected_action": [user.pk], "confirm": "yes"}
        response = admin_client.post(url, data, follow=True)
        messages_text = [str(m) for m in response.context["messages"]]
